// Iteration 1 | Count Repetition // DONE
const repeatedWords = [
  "machine",
  "matter",
  "subset",
...
// Iteration 2 | Number Sequence  // DONE
...
// Iteration 3 | Multiply for Each // DONE
...
function multiplyBy(arrayOfNumbers, multiplier) {
  ...
  arrayOfNumbers.forEach(function (number) {
    arrayMultiply.push(number * multiplier);
  });
...
function filterOut(originalArray, arrayToFilter) {
  let newArray = [];
  ...
  if (originalArray.length === 0) {
    ...
    newArray = original.filter((item) => !toRemove.includes(item));

    return newArray;
  }
}

// console.log(filterOut(testingArray, toRemove));
...
function greatestProduct(matrix) {
  ...
  matrix.forEach(function (number) {
